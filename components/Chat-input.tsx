  ...
  return (
    <ChatInputContext.Provider value={contextValue}>
      <div
        className={cn(
          variant === 'default' &&
            'flex flex-col items-end w-full p-2 rounded-2xl border border-gray-600  bg-transparent focus-within:ring-1 focus-within:ring-ring focus-within:outline-none',
...
function ChatInputSubmit({
  ...
  if (loading && onStop) {
    return (
      <Button
        onClick={onStop}
        className={cn(
          'shrink-0 rounded-full p-1.5  border border-zin dark:border-zinc-600',
          className
        )}
        {...props}
      >
  ...
  return (
    <Button
      className={cn(
        'shrink-0 rounded-full  h-fit border dark:border-zinc-600',
        className
      )}
      disabled={isDisabled}
      onClick={(event) => {
